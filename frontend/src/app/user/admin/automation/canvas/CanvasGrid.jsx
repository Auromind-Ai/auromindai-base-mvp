...
function CanvasGrid({
  ...
  return (
    <section
      ...
      style={{
        top: zenMode ? '0px' : (flowValidation.errors.length > 0 || flowValidation.warnings.length > 0 ? '80px' : '48px'),
        touchAction: 'none'
      }}
      ref={canvasRef}
      onWheel={handleWheel}
          ...
          {nodes.map(node => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  scale: isDraggingThis ? 1.08 : (isActive ? 1.04 : 1),
                  zIndex: isDraggingThis ? 150 : (isActive ? 100 : 10)
                }}
                className={`w-52 pointer-events-auto bg-[#0f0f18] backdrop-blur-none md:backdrop-blur-xl border rounded-2xl p-5 shadow-xl cursor-grab active:cursor-grabbing transition-colors duration-200 ${
                  isDraggingThis
                    ? 'border-violet-400 ring-2 ring-violet-500/50 shadow-[0_0_25px_rgba(139,92,246,0.45)]'
                    : isActive
                    ? 'border-violet-500/60 shadow-[0_0_20px_rgba(139,92,246,0.15)]'
                    : isDisconnectedNode
                    ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]'
                    : node.type === 'trigger'
                    ? 'border-[#71D7A3] shadow-[2px_2px_10px_-4px_#71D7A3]'
                    : 'border-[#814AC8] shadow-[2px_2px_10px_-2px_#814AC8]'
                }`}
              >
                {/* Moving indicator on active touch drag */}
                {isDraggingThis && (
                  <div className="absolute -top-7 left-1/2 -translate-x-1/2 flex items-center gap-1 px-3 py-1 rounded-full bg-violet-500/30 border border-violet-400/60 whitespace-nowrap shadow-lg animate-pulse">
                    <span className="text-[9px] font-black text-violet-200 uppercase tracking-widest">Moving Step</span>
                  </div>
                )}

                  {!isDraggingThis && delayLabel && (
                  ...
                  {node.type === 'trigger' && (
                    <>
                      ...
                      <button
                        ...
                        className="w-full rounded-xl border border-[#71D7A3] bg-[#140D1F] px-3 py-2.5 text-xs font-semibold text-white text-center hover:bg-[#140D1F]/80 transition cursor-pointer"
                      >
                        Preview
                      </button>
                    </>
                  ...
                  {node.type === 'action' && !isMultiPathNode(node) && (
                    <>
                      ...
                      <button
                        data-no-drag
                        onClick={(e) => { e.stopPropagation(); setPreviewNode(node); }}
                        className="w-full rounded-xl border border-[#814AC8] bg-[#140D1F] px-3 py-2.5 text-xs font-semibold text-white text-center hover:bg-[#140D1F]/80 transition cursor-pointer"
                      >
                        Preview
                      </button>
                    </>
                    ...
                    <div className="mb-4 space-y-2">
                      {nodeButtons.map((button, index) => {
                        ...
                        return (
                          <div
                            key={button.id}
                            data-button-id={handleId}
                            ref={(el) => {
                              if (el) {
                                if (!buttonOffsetsRef.current[node.id]) buttonOffsetsRef.current[node.id] = {};
                                const offset = el.offsetTop + el.offsetHeight / 2;
                                if (button.id) buttonOffsetsRef.current[node.id][button.id] = offset;
                                if (button.value) buttonOffsetsRef.current[node.id][button.value] = offset;
                                if (handleId) buttonOffsetsRef.current[node.id][handleId] = offset;
                              }
                            }}
                          ...
                          >
                            ...
                            <div className="absolute -right-2 top-1/2 -translate-y-1/2 z-20">
                              <motion.div
                                ...
                                onPointerDown={(e) => handlePortPointerDown(e, node.id, handleId, (index + 1) * 80)}
                                className="w-6 h-6 bg-indigo-500 rounded-full border-[3px] border-[#020408] shadow-[0_0_14px_rgba(99,102,241,0.55)] cursor-crosshair flex items-center justify-center select-none"
                              >
                  ...
                  {nodeButtons.length > 0 && (
                    <button
                      data-no-drag
                      onClick={(e) => { e.stopPropagation(); setPreviewNode(node); }}
                      className="w-full rounded-xl border border-[#814AC8] bg-[#140D1F] px-3 py-2.5 text-xs font-semibold text-white text-center hover:bg-[#140D1F]/80 transition mb-4 cursor-pointer"
                    >
                      Preview
                    </button>
                  )}
                  ...
                  {isConditionNode(node) && (
                    <button
                      data-no-drag
                      onClick={(e) => { e.stopPropagation(); setPreviewNode(node); }}
                      className="w-full rounded-xl border border-[#814AC8] bg-[#140D1F] px-3 py-2.5 text-xs font-semibold text-white text-center hover:bg-[#140D1F]/80 transition mt-1 cursor-pointer"
                    >
                      Preview
                    </button>
                  )}
                  ...
                  {nodeButtons.length === 0 && nodeBranches.length === 0 && node.config?.type !== 'brain_query' && (
                    <motion.div
                      ...
                      className={`absolute -right-2 top-1/2 -translate-y-1/2 w-6 h-6 bg-violet-500 rounded-full border-[3px] border-[#0d0d12] shadow-[0_0_12px_rgba(139,92,246,0.5)] cursor-crosshair z-20 flex items-center justify-center group/port select-none ${isActive ? 'scale-110' : ''}`}
                    >
                      <Plus size={11} className="text-white opacity-90 md:opacity-0 md:group-hover/port:opacity-100 transition-opacity" />
                    </motion.div>
                  )}
