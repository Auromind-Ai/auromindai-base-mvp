...
const GET_STARTED_CARDS = [
    {
        ...
        cardBg: "linear-gradient(180deg, rgba(7,0,18,0.92) 0%, rgba(7,0,18,0.96) 72%, rgba(139,92,246,0.28) 100%)",
    ...
    },
    {
        ...
        cardBg: "linear-gradient(180deg, rgba(7,0,18,0.92) 0%, rgba(7,0,18,0.96) 72%, rgba(16,185,129,0.28) 100%)",
        borderColor: "rgba(255,255,255,0.08)",
    },
    {
        icon: ImageIcon,
        ...
        cardBg: "linear-gradient(180deg, rgba(7,0,18,0.92) 0%, rgba(7,0,18,0.96) 72%, rgba(249,115,22,0.28) 100%)",
        borderColor: "rgba(255,255,255,0.08)",
    },
    {
        icon: ListTodo,
        ...
        cardBg: "linear-gradient(180deg, rgba(7,0,18,0.92) 0%, rgba(7,0,18,0.96) 72%, rgba(59,130,246,0.28) 100%)",
        borderColor: "rgba(255,255,255,0.08)",
    }
];
...
export default function AuromindAIPage() {
    ...
    const handleCreateSession = async () => {
        ...
        sessionStorage.removeItem("last_session_id"); // ← இந்த line மட்டும் add பண்ணு
        setMessages([]);
        setInputValue('');
    ...
    // ── Derived condition for which screen to show ───────────────────────────
    // Hero: no messages AND not loading a session
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {!isChatStarted ? (
                                    ...
                                    >
                                        ...
                                        {/* Prompt Input */}
                                        <motion.div layoutId="chat-input-container" className="w-full max-w-3xl">
                                            <div className="ai-input-box bg-[#070012] rounded-2xl border border-white/10 shadow-2xl transition-all duration-300 overflow-hidden">
                                                <div className="px-5 pt-4">
                                                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-purple-600/20 text-[12px] font-medium text-purple-300 border border-purple-500/25 hover:bg-purple-600/30 transition-all">
                                                        <Plus size={13} />
                                                ...
                                                <div className="flex items-center justify-between px-5 pb-4 pt-2 border-t border-white/10">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[13px] text-gray-400 hover:bg-white/5 border border-white/10 hover:border-white/20 transition-all">
                                                            <Globe size={14} />
                                                    ...
                                                    <button
                                                        ...
                                                        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${inputValue.trim() ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/30 hover:bg-purple-500' : 'bg-white/5 text-gray-700'}`}
                                                    >
                                                        <ArrowUp size={17} />
                                                    </button>
                                                </div>
                                                ...
                                                {GET_STARTED_CARDS.map((card, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="relative z-10 flex-1">
                                                            <p className="text-[13px] font-semibold text-gray-200 mb-1 leading-snug">{card.label}</p>
                                                            <p className="text-[11px] text-white/60 leading-snug">{card.description}</p>
                                                        </div>
                                                        <div className="absolute bottom-3 right-3 z-10 w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-purple-600/30 group-hover:border-purple-500/40 transition-all">
                                                            <ArrowUpRight size={12} className="text-gray-500 group-hover:text-purple-300 transition-colors" />
                                                        </div>
                                                    </motion.button>
                                                ))}
                                            </div>
                                        ...
                                        {isInitializing ? (
                                            ...
                                            <div className="flex flex-col gap-2 w-full py-8">
                                                {messages.map((msg, idx) => (
                                                    ...
                                                    >
                                                        {msg.role === 'user' ? (
                                                            <div className="bg-[#070012] text-[#efefef] rounded-2xl px-4 py-2.5 max-w-[85%] border border-purple-500/10 shadow-sm">
                                                                {editingIndex === idx ? (
                                        ...
                                        <div ref={plusRef} className="relative flex items-center px-4 py-3">
                                                        ...
                                                        {MODELS.map((model) => {
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    className="flex items-center justify-between w-full px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded-lg transition-colors"
                                                                >
                                            ...
                                            {isPlusOpen && (
                                                <div className="absolute bottom-14 left-4 bg-[#1a1a2e] border border-white/10 rounded-xl shadow-xl w-44 p-2 z-50">
                                                    <button onClick={() => { fileInputRef.current?.click(); setIsPlusOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded-lg">
                                                        <Paperclip size={16} />Attach File
                                                    </button>
                                                    <button onClick={() => setIsPlusOpen(false)} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded-lg">
                                                        <Globe size={16} />Search
                                                    </button>
                                                    <button onClick={() => setIsPlusOpen(false)} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded-lg">
                                                        <Sparkles size={16} />{selectedModel === 'auto' ? 'Auto' : 'Pro'}
                                                    </button>
                                                </div>
                                            )}
