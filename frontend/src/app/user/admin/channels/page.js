...
import { getWorkspace, authHeader, getWorkspaceIdFromToken } from '@/lib/auth';
...
export default function ChannelsPage() {
    ...
    const lastSignupData = useRef(null);
    const lastAuthCode = useRef(null);
    ...
    // Fetch channel connection statuses from backend
    const fetchChannelsStatus = useCallback(async (wsId) => {
        if (!wsId) return;
        try {
            const data = await api.get(`/api/channels/status?workspace_id=${wsId}`);
            setStatuses({
                whatsapp: data.whatsapp?.connected || false,
                instagram: data.instagram?.connected || false,
                twilio: data.twilio?.connected || false,
                gmail: false // Google/Gmail is handled separately
            });
            setConnectedInfo({
                whatsapp: data.whatsapp?.phone || '',
                instagram: data.instagram?.username || '',
                twilio: data.twilio?.phone || ''
            });
        } catch (err) {
    ...
    useEffect(() => {
        ...
        setWorkspace(ws);
        if (ws?.id) {
            fetchChannelsStatus(ws.id);
        }
    }, [fetchChannelsStatus]);

    const trySubmitWhatsApp = useCallback(async (code, wabaData) => {
        const payload = {
            code: code || lastAuthCode.current,
            phone_number_id: wabaData?.phone_number_id || lastSignupData.current?.phone_number_id,
            waba_id: wabaData?.waba_id || lastSignupData.current?.waba_id,
            redirect_uri: window.location.href.split('#')[0].split('?')[0]
        };

        console.log("trySubmitWhatsApp synchronized payload:", payload);

        if (!payload.code) {
            console.log("Waiting for authorization code from FB.login...");
            return;
        }
        if (!payload.phone_number_id || !payload.waba_id) {
            console.log("Waiting for phone number and WABA selection from popup...");
            return;
        }

        // Clear refs to prevent duplicate submissions
        lastAuthCode.current = null;
        lastSignupData.current = null;

        await connectWhatsAppToBackend(payload);
    }, [workspace?.id]);
            ...
            try {
                ...
                if (data?.type === 'WA_EMBEDDED_SIGNUP') {
                    if (data.event === 'FINISH') {
                        ...
                        lastSignupData.current = { phone_number_id, waba_id };
                        trySubmitWhatsApp(null, { phone_number_id, waba_id });
    ...
    }, [workspace?.id, trySubmitWhatsApp]);

    const startWhatsAppSignup = useCallback(() => {
        if (window.location.protocol === 'http:' && window.location.hostname === 'localhost') {
            alert("Facebook strict policy requires HTTPS for the connection popup. Redirecting you to the secure HTTPS proxy (Port 3001) now...");
            window.location.href = `https://localhost:3001${window.location.pathname}`;
            return;
        }
        ...
        console.log("WhatsApp Embedded Signup configs:", { appId, configId });

        if (!appId || !configId) {
            alert(
                "Missing Meta/Facebook Integration Configuration!\n\n" +
                "To connect a WhatsApp Business API account, please add the following keys to your 'frontend/.env.local' file:\n\n" +
                `NEXT_PUBLIC_FB_APP_ID=${appId || '1965491380714102'}\n` +
                `NEXT_PUBLIC_META_CONFIG_ID=your_whatsapp_embedded_signup_configuration_id\n\n` +
                "Once added, please restart the frontend server (npm run dev) so the environment variables take effect."
            );
            setConnecting(null);
            return;
        }
        ...
        if (!window.FB) {
            alert(
                "Facebook SDK is not loaded yet or was blocked by the browser.\n\n" +
                "Please verify:\n" +
                "1. You have a stable internet connection.\n" +
                "2. You are not using an ad blocker or privacy extension that blocks Facebook's SDK (connect.facebook.net).\n" +
                "3. Try refreshing the page and waiting 3-5 seconds for the SDK to load."
            );
            setConnecting(null);
            return;
        }
        ...
        try {
            window.FB.login(
                (response) => {
                    ...
                    if (response.authResponse?.code) {
                        lastAuthCode.current = response.authResponse.code;
                        trySubmitWhatsApp(response.authResponse.code, null);
                    } else {
                        console.warn("FB.login failed or was cancelled by user:", response);
                        setConnecting(null);
                    }
                },
                {
            ...
            console.error("Error executing window.FB.login:", error);
            alert(`Facebook Login initialization error: ${error.message}`);
            setConnecting(null);
        }
    }, [WA_CONFIG_ID, trySubmitWhatsApp]);

    async function connectWhatsAppToBackend(payload) {
        try {
            const workspaceId = workspace?.id || getWorkspace()?.id || getWorkspaceIdFromToken();
            if (!workspaceId) {
            ...
            const data = await api.post('/api/whatsapp/connect', { ...payload, workspace_id: workspaceId });
            if (data.status === 'connected') {
                setStatuses(prev => ({ ...prev, whatsapp: true }));
                setConnectedInfo(prev => ({ ...prev, whatsapp: data.display_number || data.phone_number }));
    ...
    const connectInstagramToBackend = async (code) => {
        try {
            const data = await api.post('/api/instagram/connect', { code, workspace_id: workspace?.id });
            if (data.status === 'connected') {
    ...
    const submitTwilio = async () => {
        ...
        try {
            const data = await api.post('/twilio/connect', { sid, token, phone, workspace_id: workspace?.id });
            if (data.status === 'connected') {
    ...
    const handleConnect = (id, force = false) => {
        if (statuses[id] && !force) return;
                                ...
                                <div className="px-6 py-4 border-t border-[#141414] flex items-center justify-between">
                                    ...
                                    {isConnected ? (
                                        <button
                                            ...
                                            className="group/btn flex items-center gap-2 px-4 py-2 rounded-xl bg-green-500/10 hover:bg-green-500/20 border border-green-500/30 hover:border-green-500 text-green-400 hover:text-green-300 text-[13px] font-medium transition-all hover:shadow-[0_0_15px_rgba(34,197,94,0.2)]"
                                        >
                                            <Check size={14} className="block group-hover/btn:hidden" />
                                            <ChevronRight size={14} className="hidden group-hover/btn:block text-green-300" />
                                            <span className="block group-hover/btn:hidden">Connected</span>
                                            <span className="hidden group-hover/btn:block">Reconnect</span>
                                        </button>
                                    ) : (
                                        <button
