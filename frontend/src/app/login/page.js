...
export default function LoginPage() {
    ...
    const handleQuickTestLogin = async () => {
        setError('');
        setLoading(true);
        const testEmail = 'tester@auromind.ai';
        setEmail(testEmail);
        try {
            try {
                await api.sendOTP(testEmail, 'login');
            } catch (e) {
                // Ignore send error for test bypass
            }
            
            const data = await api.verifyOTP(testEmail, '123456', 'login');
            if (!data?.access_token) throw new Error('Verification failed');
            ...
            <div className="w-full lg:w-[45%] flex flex-col justify-between p-8 sm:p-12 relative z-10 border-l border-white/5 bg-[#050505]">
                ...
                <div className="w-full max-w-[380px] mx-auto relative z-10">
                    ...
                    >
                        ...
                        {/* Test Environment Helper Card */}
                        <div className="mt-8 p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 backdrop-blur-md relative overflow-hidden group text-left">
                            <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/5 to-transparent pointer-events-none" />
                            <div className="flex items-start gap-3 relative z-10">
                                <div className="text-indigo-400 mt-0.5">💡</div>
                                <div className="space-y-2">
                                    <h4 className="text-[11px] font-bold text-indigo-300 uppercase tracking-wider">Test Environment Active</h4>
                                    <p className="text-[11px] text-white/50 leading-relaxed">
                                        Use any email address to log in (unregistered emails will be auto-created). Enter verification code <code className="text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded font-mono font-bold">123456</code>.
                                    </p>
                                    <button
                                        type="button"
                                        onClick={handleQuickTestLogin}
                                        disabled={loading}
                                        className="mt-1 flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-fuchsia-600 hover:from-indigo-500 hover:to-fuchsia-500 disabled:opacity-50 text-white rounded-xl text-[11px] font-bold transition-all shadow-md active:scale-95"
                                    >
