    ...
    >
            ...
            {config.heroVideo ? (
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between px-1">
                  ...
                  <span className="text-[10px] text-emerald-400 font-semibold">Meta Approved</span>
                </div>
                <DocumentationScreenshot
              ...
              <div className={stretchBenefits ? 'grid gap-3 lg:flex-1 lg:auto-rows-fr' : 'space-y-3'}>
                {config.benefits.map((b, idx) => (
                  <div
                    ...
                    className="p-4 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-all flex items-start gap-3"
                  >
                    <div className="w-7 h-7 rounded-md bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 font-bold text-xs shrink-0 mt-0.5">
                      {idx + 1}
                    </div>
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
            ...
            {config.beforeYouStart.map((item, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-md bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 text-xs font-bold shrink-0">
                    {idx + 1}
                  </div>
                  <h3 className="text-sm font-semibold text-white">
                    {item.title}
      ...
      {config.connectionRules && config.connectionRules.length > 0 && (
        <section id="connection-rules" className="scroll-mt-24 pt-4 border-t border-white/10 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="self-start sm:self-auto px-3.5 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs font-semibold flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {config.architecture.stages.map((st) => (
                  <div key={st.number} className="p-3.5 rounded-xl border border-white/5 bg-black/40 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-bold bg-violet-500/10 px-2 py-0.5 rounded border border-violet-500/20">
                        S{st.number}
                      </span>
      ...
      {!isFlowPage && config.setupSteps && config.setupSteps.length > 0 && (
        <section id="configuration" className="scroll-mt-24 pt-4 border-t border-white/10 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <span className="text-[11px] tracking-widest text-pink-400 font-bold block mb-1">
            ...
            {isIntegration && (
              <div className="self-start sm:self-auto px-3.5 py-1.5 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-300 text-xs font-semibold flex items-center gap-1.5">
              ...
              {config.setupSteps.map((s, idx) => {
                ...
                const textBlock = (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2.5">
                      <span className="px-2.5 py-1 rounded-lg bg-pink-500/15 border border-pink-500/30 text-pink-300 text-xs font-bold">
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 ${alignPanels ? 'items-stretch' : 'items-start'}`}>
              <div className="lg:col-span-6 space-y-3 pt-2">
                ...
                {config.setupSteps.map((s) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="w-5 h-5 rounded-md bg-pink-500/10 border border-pink-500/20 flex items-center justify-center text-pink-400 text-xs">
                        {s.step}
                      </span>
                      <h3 className="text-sm font-semibold text-white">
                        {s.title}
              ...
              <div className={`lg:col-span-6 ${alignPanels ? 'flex flex-col gap-4' : 'space-y-4'}`}>
                ...
                {!isInbox && !config.hideWorkflowStages && config.architecture?.stages && config.architecture.stages.length > 0 && (
                  <div className={`p-4 rounded-xl border border-white/10 bg-white/[0.02] ${alignPanels ? 'flex flex-col gap-3 lg:flex-1' : 'space-y-2.5'}`}>
                    <span className="text-[11px] uppercase tracking-widest text-zinc-400 block">
        ...
        <section id="verification" className="scroll-mt-24 pt-4 border-t border-white/10">
          <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 ${isInbox ? 'items-stretch' : 'items-center'}`}>
            ...
            <div className={`${isInbox ? 'lg:col-span-7 flex flex-col gap-4' : 'lg:col-span-6 space-y-4'} order-1 lg:order-2`}>
              <div>
                <span className="text-[11px] tracking-widest text-emerald-400 font-bold block mb-1">
                  Expected Result
                </span>
                <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
      ...
      {config.troubleshooting && config.troubleshooting.length > 0 && (
        <section id="troubleshooting" className="scroll-mt-24 pt-4 border-t border-white/10 space-y-6">
          <div className="space-y-1">
            <span className="text-[11px] tracking-widest text-sky-400 font-bold block">
              FAQ &amp; Troubleshooting
            </span>
            <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
