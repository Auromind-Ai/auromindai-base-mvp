...
import CrmControls, { cleanFilters } from '@/components/leads/CrmControls';
...
function LeadsPanel({ 
    ...
    const filterOptions = upgraded ? FILTER_OPTIONS : FILTER_OPTIONS.filter(option => option.id !== 'gmail');
    const activeFilterObj = filterOptions.find(f => f.id === selectedFilter) || FILTER_OPTIONS[0];
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-zinc-500 font-medium px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.06]">
                            {upgraded ? `${totalCount.toLocaleString()} leads found` : leads.length}
                        </span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {loading && leads.length === 0 ? (
                    ...
                    leads.map(lead => {
                        ...
                        const scoreColor = lead.lead_tier === 'hot' ? theme.scoreColor : lead.lead_tier === 'warm' ? 'text-sky-400' : 'text-orange-400';
                        return (
                            <div key={lead.id} className="flex items-start gap-2">
                            {upgraded && <input type="checkbox" aria-label={`Select ${lead.name} for export`} className="mt-5 accent-violet-500" checked={selectedIds.includes(lead.id)} onChange={() => onToggleSelection(lead.id)} />}
                            <button
            ...
            <div className={`flex items-center justify-between px-3 sm:px-5 md:px-6 py-2.5 sm:py-3.5 bg-[#0A0A10] border-b border-white/[0.08] flex-shrink-0 relative z-10 transition-all ${theme.headerBorder}`}>
                ...
                <div className="flex items-center gap-2 sm:gap-3.5 min-w-0 flex-1 pr-2">
                    ...
                    <div className="min-w-0 flex-1">
                        {/* Line 1: Name, Verified Badge, Separator, Status Pill */}
                        ...
                        {/* Line 2: Tier & Labels Badges */}
                        <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 mt-1">
                            {(() => {
                                const t = leadDetail?.lead_tier || lead?.lead_tier || 'cold';
                                const badge = TIER_BADGES[t.toLowerCase()] || TIER_BADGES.cold;
                                return (
                                    <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold border ${badge.bg} ${badge.textCls} ${badge.border} shrink-0`}>
                                        {badge.text}
                                    </span>
                                );
                            })()}
                            {(leadDetail?.labels || lead?.labels || []).slice(0, 2).map(lbl => {
                                const config = AGENT_LABELS[lbl];
                                if (!config) return null;
                                return (
                                    <span key={lbl} className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-semibold border bg-opacity-30 ${config.activeBg} ${config.textCls} shrink-0`}>
                                        {config.emoji} {lbl}
                                    </span>
                                );
                            })}
                ...
                {/* Right Side: Lead Score & Action Buttons */}
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 shrink-0">
                    {/* Lead Score Container */}
                    <div className="text-right">
                        <div className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-wider font-bold text-[#8E8E93] leading-none mb-0.5 sm:mb-1">LEAD SCORE</div>
                        <div className={`text-sm sm:text-lg md:text-xl lg:text-2xl font-black leading-none ${theme.scoreColor}`}>
                            {lead.score}%
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2">
                        ...
                        {onToggleOverview && (
                            <button
                                ...
                                className="w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-xl border border-[#7C4DFF]/30 bg-[#7C4DFF]/15 text-[#7C4DFF] hover:bg-[#7C4DFF]/25 flex items-center justify-center transition-all shrink-0 cursor-pointer min-[1601px]:hidden"
                        ...
                        <button
                            ...
                            className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-xl border flex items-center justify-center transition-all shrink-0
                                ${isFavorite
                                    ? 'bg-yellow-400/10 border-yellow-400/20 text-yellow-400 hover:bg-yellow-400/20 shadow-md'
                                    : 'bg-[#11111A] border-[#252535] text-zinc-500 hover:text-amber-400 hover:bg-white/5'
                                }`}
                            title={isFavorite ? 'Remove from favorites' : 'Add to favorites'}
                        >
                            <Star size={14} className="sm:w-4 sm:h-4" fill={isFavorite ? 'currentColor' : 'none'} />
                        </button>
                    </div>
                </div>
            </div>
...
function RightPanel({ lead, details, history, loadingHistory, onBackToChat, isTabletView, isMobileView, isIpadView, isDesktopDrawer }) {
    const [isTimelineExpanded, setIsTimelineExpanded] = useState(false);


    const isOverlayOrTablet = isTabletView || isMobileView || isIpadView || isDesktopDrawer || Boolean(onBackToChat);
        ...
        <div className={`${isOverlayOrTablet ? 'flex-1 w-full flex' : 'hidden xl:flex w-[380px]'} flex-shrink-0 flex-col bg-[#0D0D17] border-l border-white/[0.06] overflow-y-auto`}>
            ...
            {isOverlayOrTablet && (
                <div className="flex items-center justify-between px-5 py-3.5 bg-[#0D0D17] border-b border-white/[0.06] flex-shrink-0">
                    ...
                    >
                        <span>← {isMobileView ? 'Back to Conversation' : 'Back to Chat'}</span>
                    </button>
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-white">Lead Overview</span>
                        {onBackToChat && (
                            <button
                                onClick={onBackToChat}
                                className="p-1 rounded-lg text-zinc-400 hover:text-white hover:bg-white/5 transition-all cursor-pointer ml-1"
                                title="Close"
                            >
                                <X size={16} />
                            </button>
...
function WorkspaceContent({ upgraded, workspaceId }) {
    ...
    useEffect(() => {
        // Reset pagination and selection when synchronizing with a new server query.
    ...
    }, [debouncedSearch, selectedFilter, filters]);
    ...
    const fetchSelectedLeadData = async (leadId) => {
        ...
        try {
                ...
                setLeads(prev => prev.map(l => {
                    if (l.id === leadId) {
                        return {
                            ...
                            labels: normalizedDetail.labels,
                            assigned_to: normalizedDetail.assigned_to
                        };
                    }
                    return l;
                }));
            ...
            if (activeFetchesRef.current[leadId] === controller) {
                setDetailsLoading(prev => ({ ...prev, [leadId]: false }));
                setHistoryLoading(prev => prev === leadId ? null : prev);
    ...
    useEffect(() => {
        // Fetch only the selected lead; loading state belongs to this request.
        // eslint-disable-next-line react-hooks/set-state-in-effect
        if (selectedLeadId && (!upgraded || section === 'scoring')) fetchSelectedLeadData(selectedLeadId);
    }, [selectedLeadId, upgraded, section]);
    useEffect(() => {
        const active = activeFetchesRef.current;
    ...
    const handleLeadCreated = (newLeadResponse) => {
        ...
        initialLeadRef.current = newLead.id;
                ...
                /> : <div className="flex-1 flex overflow-hidden relative">
                    ...
                    <div className="hidden xl:flex flex-1 w-full overflow-hidden relative">
                        <LeadsPanel
                            ...
                            onFilterChange={changeQuickFilter}
                            selectedIds={selectedIds}
                            onToggleSelection={toggleSelection}
                            upgraded={upgraded}
                        />
                        <div className="flex-1 flex overflow-hidden">
                            ...
                            <div className="hidden min-[1601px]:flex flex-shrink-0">
                                <RightPanel
                                    key={selectedLeadId}
                                    lead={selectedLead}
                            ...
                            {desktopDrawerOpen && (
                                <>
                                    ...
                                    <motion.div
                                        ...
                                        className="fixed top-0 right-0 bottom-0 z-[90] w-[380px] max-w-[90vw] flex flex-col min-[1601px]:hidden shadow-2xl"
                                    >
                                        <RightPanel
                                    key={selectedLeadId}
                                            lead={selectedLead}
                                            ...
                                            history={historyLogs[selectedLeadId]}
                                            loadingHistory={historyLoading === selectedLeadId}
                                            onBackToChat={() => setDesktopDrawerOpen(false)}
                                            isDesktopDrawer={true}
                                        />
                                    </motion.div>
                                </>
                            )}
