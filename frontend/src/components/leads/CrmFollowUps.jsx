  ...
  const removeFollowUp = (lead) => {
    ...
    const performRemoval = async () => {
      ...
      try {
        ...
        showToast?.(`${lead.name || "Lead"} removed from follow-up`, "success");
      } catch (err) {
        const msg = err.message || "Unable to remove this follow-up lead. Please try again.";
        setRemoveError(msg);
        showToast?.(msg, "error");
      } finally {
        setRemovingId(null);
    ...
    if (showConfirm) {
      showConfirm({
        ...
        message: `Remove ${lead.name || "this lead"} from follow up? The lead will remain in CRM.`,
        confirmText: "Remove",
        cancelText: "Cancel",
        type: "danger",
