...
export default function Nova({ hue = 0, hoverIntensity = 0.2, rotateOnHover = true }) {
  ...
  const frag = /* glsl */ `
    ...
    float snoise3(vec3 p) {
      ...
      float k1_val = (p.x + p.y + p.z) * K1;
      vec3 i = floor(p + vec3(k1_val, k1_val, k1_val));
      float k2_val = (i.x + i.y + i.z) * K2;
      vec3 d0 = p - (i - vec3(k2_val, k2_val, k2_val));
      vec3 e = step(vec3(0.0, 0.0, 0.0), d0 - d0.yzx);
      vec3 i1 = e * (vec3(1.0, 1.0, 1.0) - e.zxy);
      vec3 i2 = vec3(1.0, 1.0, 1.0) - e.zxy * (vec3(1.0, 1.0, 1.0) - e);
      vec3 d1 = d0 - (i1 - vec3(K2, K2, K2));
      vec3 d2 = d0 - (i2 - vec3(K1, K1, K1));
      vec3 d3 = d0 - vec3(0.5, 0.5, 0.5);
      vec4 h = max(vec4(0.6, 0.6, 0.6, 0.6) - vec4(
        dot(d0, d0),
        dot(d1, d1),
        dot(d2, d2),
        dot(d3, d3)
      ...
      vec4 n = h * h * h * h * vec4(
        ...
        dot(d1, hash33(i + i1)),
        dot(d2, hash33(i + i2)),
        dot(d3, hash33(i + vec3(1.0, 1.0, 1.0)))
      );
