...
export default function InteractiveBrainSection() {
  ...
  const prevStageData = useRef(stageData);
...
useEffect(() => {
  const observer = new IntersectionObserver(
    ([entry]) => {
      ...
      // section visible ana udane lock pannu
      if (visible && !unlocking) {
  setIsLocked(true);
}

if (!visible && currentStage === 4 && unlocking) {
  setIsLocked(false);
}
    },
    {
  ...
  (direction) => {
    ...
    if (direction === "down") {
      ...
      // Stage 4 la irundha next scroll allow page move
      setIsLocked(false);
    ...
    if (direction === "up") {
      ...
      // Stage 1 la irundha next upward scroll allow previous section
      setIsLocked(false);
  ...
  const showVideo = currentStage >= 2 && stageData.video;
