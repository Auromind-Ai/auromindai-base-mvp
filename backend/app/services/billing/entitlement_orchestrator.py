...
class EntitlementOrchestrator:
    ...
    def _change_subscription_plan(
        ...
        if isinstance(workspace_id, str):
            workspace_id = uuid.UUID(workspace_id)
        if isinstance(new_plan_id, str):
            new_plan_id = uuid.UUID(new_plan_id)
        ...
        if active_sub:
            ...
            active_sub.current_period_end = datetime.now(timezone.utc) + timedelta(
                days=365 if getattr(new_plan, "billing_cycle", "monthly") == "yearly" else 30
            )
            ...
            new_sub = Subscription(
                ...
                current_period_start=datetime.now(timezone.utc),
                current_period_end=datetime.now(timezone.utc) + timedelta(
                    days=365 if getattr(new_plan, "billing_cycle", "monthly") == "yearly" else 30
                ),
                provider="system",
