...
<body style="margin: 0; padding: 0; background-color: #F8FAFC; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
    ...
    def seed_default_templates(cls, db: Session) -> int:
        ...
        baseline_templates = [
            ...
            {"category": "Payments & Credits", "template_key": "ai_credits_purchase_success", "name": "AI Credits Purchase Confirmation", "channel": "both", "title": "AI Credits Added Successfully", "subject": "AI Credits Confirmed: {{credits_added}} Credits Added", "message": "Hi {{user_name}},\n\nYour purchase of {{credits_added}} AI Credits (Amount: {{amount}}) for {{workspace_name}} was successful.\n\nCurrent Available Balance: {{current_balance}} AI Credits\nInvoice ID: {{invoice_id}}", "is_active": True},
            {"category": "Payments & Credits", "template_key": "wcc_wallet_recharge_success", "name": "WhatsApp Wallet Recharge Confirmation", "channel": "both", "title": "WhatsApp Wallet Recharged Successfully", "subject": "WhatsApp Recharge Confirmed: {{amount_added}} Added", "message": "Hi {{user_name}},\n\nYour WhatsApp Wallet recharge of {{amount_added}} (Amount: {{amount}}) for {{workspace_name}} was successful.\n\nCurrent Available Balance: {{current_balance}}\nInvoice ID: {{invoice_id}}", "is_active": True},
            {"category": "Broadcast & Workflow", "template_key": "flow_pack_purchase_success", "name": "Flow Pack Purchase Confirmation", "channel": "both", "title": "Flow Packs Added Successfully", "subject": "Flow Pack Confirmed: {{flows_added}} Flows Added", "message": "Hi {{user_name}},\n\nYour purchase of {{flows_added}} Flow Executions (Amount: {{amount}}) for {{workspace_name}} was successful.\n\nCurrent Available Balance: {{current_balance}}\nTotal Monthly Quota: {{total_quota}}\nInvoice ID: {{invoice_id}}", "is_active": True},
            {"category": "Payments & Credits", "template_key": "payment_cancelled", "name": "Payment Checkout Cancelled / Incomplete", "channel": "both", "title": "Subscription Upgrade Incomplete", "subject": "Did something go wrong with your Orbion Agents subscription?", "message": "Hi {{user_name}},\n\nWe noticed you started upgrading to {{plan_name}} but didn't finish the payment.\n\nAmount: {{amount}}\nWorkspace: {{workspace_name}}\n\nIf you encountered any issue or have questions about our plans, simply click below to resume your checkout or reach out to our team.", "is_active": True},
            ...
            # Broadcast & Workflow
            {"category": "Broadcast & Workflow", "template_key": "broadcast_completed", "name": "Outbound Broadcast Completed", "channel": "both", "title": "Broadcast Campaign Completed", "subject": "Campaign Finished: {{broadcast_name}} ({{delivered}} delivered)", "message": "Hi {{user_name}},\n\nYour outbound broadcast campaign '{{broadcast_name}}' has completed.\n\nPerformance Summary:\n• Total Targeted: {{total_sent}}\n• Successfully Delivered: {{delivered}}\n• Failed / Bounced: {{failed}}\n• Read Receipts: {{read}}\n\nView the detailed delivery analytics below.", "is_active": True},
            ...
            # Reports
            {"category": "Reports", "template_key": "daily_dashboard_summary", "name": "Daily Performance Summary", "channel": "email", "title": "Daily Workspace Digest", "subject": "Daily Digest: {{workspace_name}} Summary for {{date}}", "message": "Hi {{user_name}},\n\nHere is your daily activity summary for {{workspace_name}} on {{date}}:\n\n• New Leads Captured: {{new_leads}}\n• Deals Converted: {{conversions}}\n• Total Revenue: {{revenue}}\n• Unanswered Messages: {{unanswered_messages}}\n• AI Credits Consumed (Today): {{credits_used}}\n• Available AI Credits: {{credit_balance}}\n\nLog in to your admin dashboard to view real-time charts.", "is_active": True},
            {"category": "Reports", "template_key": "weekly_performance_report", "name": "Weekly Business Performance Report", "channel": "email", "title": "Weekly Performance Report", "subject": "Weekly Report: {{workspace_name}} ({{date_range}})", "message": "Hi {{user_name}},\n\nHere is your weekly business performance report for {{workspace_name}} ({{date_range}}):\n\n• Funnel Performance: {{funnel_stats}}\n• Top Sales Agents: {{top_agents}}\n• Active Automations: {{active_workflows}}\n• Weekly AI Credits Used: {{weekly_credits_used}}\n• Available AI Credits: {{credit_balance}}\n\nLog in to your dashboard to explore full analytics.", "is_active": True},
        ...
        baseline_rules = [
            ...
            {"event_name": "ai_credits.purchased", "template_key": "ai_credits_purchase_success", "recipient_roles": ["workspace_owner", "billing_contact"], "channels": ["email", "in_app"], "delay_minutes": 0},
            {"event_name": "wcc_wallet.recharged", "template_key": "wcc_wallet_recharge_success", "recipient_roles": ["workspace_owner", "billing_contact"], "channels": ["email", "in_app"], "delay_minutes": 0},
            {"event_name": "flow_pack.purchased", "template_key": "flow_pack_purchase_success", "recipient_roles": ["workspace_owner", "billing_contact"], "channels": ["email", "in_app"], "delay_minutes": 0},
            {"event_name": "payment.cancelled", "template_key": "payment_cancelled", "recipient_roles": ["workspace_owner", "billing_contact"], "channels": ["email", "in_app"], "delay_minutes": 0},
