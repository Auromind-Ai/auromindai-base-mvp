...
def create_template(data: TemplateCreate, db: Session = Depends(get_db)):
    ...
    if not workspace.meta_waba_id or not workspace.meta_access_token:
        raise HTTPException(400, "Meta WhatsApp Business API credentials (WABA ID or Access Token) are not configured for this workspace. Please update them in Settings.")
    ...
    if meta_response.get("error"):
        ...
        db.commit()
        error_msg = meta_response.get("error", {}).get("message", "Unknown error from Meta")
...
def build_components(data):
    ...
    if data.type == "TEXT":
        ...
        components.append({
            ...
            "format": "IMAGE",
            "example": {
                "header_handle": ["https://via.placeholder.com/300"]
            }
        })
        ...
        components.append({
            ...
            "format": "VIDEO",
            "example": {
                "header_handle": ["https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4"]
            }
        })
...
def validate_category(data):
    if data.category == "AUTHENTICATION":
        if "{{1}}" not in data.message:
            raise HTTPException(400, "Authentication templates must include OTP variable {{1}}")
