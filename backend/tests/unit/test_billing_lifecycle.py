...
def test_granular_admin_reset_credits(db):
    """Test granular AI credit resets: target='included', target='purchased', target='all'."""

    EntitlementService.seed_default_entitlements(db)

    # Setup Workspace with Pro plan (250,000 included credits)
    ws = Workspace(id=uuid.uuid4(), name="Granular Reset Credits WS", plan_type="pro")
    db.add(ws)
    db.commit()
    ...
    # Initial entitlement provisioning (250,000 included)
    ent = EntitlementService.get_workspace_entitlement(db, ws.id)
    EntitlementOrchestrator.provision_resources(db, ws.id, ent)
    
    # Add 5,000 purchased credits
    token_service = BillingService().token_service
    token_service.grant_purchased_credits(
        db=db,
        workspace_id=str(ws.id),
        credits=5000.0,
        payment_id=f"pay_{uuid.uuid4()}",
        gateway_order_id=f"ord_{uuid.uuid4()}",
        description="Top-up 5000 credits"
    )
    db.commit()

    # Verify initial balances: 250,000 included + 5,000 purchased = 255,000 total
    summary = BillingService().get_credit_summary(db, ws.id)
    assert summary["credits_balance"] == 255000.0
    assert summary["included_remaining"] == 250000.0
    assert summary["purchased_remaining"] == 5000.0

    # 1. Reset Included Only
    import asyncio
    asyncio.run(reset_credits(ws.id, ResetResourceRequest(target="included", reason="Reset Included Only"), db=db, admin_user="test_admin"))

    summary = BillingService().get_credit_summary(db, ws.id)
    assert summary["credits_balance"] == 5000.0
    assert summary["included_remaining"] == 0.0
    assert summary["purchased_remaining"] == 5000.0

    # 2. Reset Purchased Only
    asyncio.run(reset_credits(ws.id, ResetResourceRequest(target="purchased", reason="Reset Purchased Only"), db=db, admin_user="test_admin"))
    ...
    # 3. Renew plan credits to restore 250,000 included and top-up 10,000
    EntitlementOrchestrator.renew_subscription(db, ws.id)
    token_service.grant_purchased_credits(
        db=db,
        workspace_id=str(ws.id),
        ...
        payment_id=f"pay_{uuid.uuid4()}",
        gateway_order_id=f"ord_{uuid.uuid4()}",
        description="Top-up 10000 credits"
    )
    db.commit()

    summary = BillingService().get_credit_summary(db, ws.id)
    assert summary["credits_balance"] == 260000.0

    # 4. Reset All
    asyncio.run(reset_credits(ws.id, ResetResourceRequest(target="all", reason="Reset All Credits"), db=db, admin_user="test_admin"))
...
def test_granular_admin_reset_wallet_and_subsequent_recharge(db):
    """Test granular WCC wallet reset and verify subsequent recharge does NOT resurrect old balance."""


    EntitlementService.seed_default_entitlements(db)

    ws = Workspace(id=uuid.uuid4(), name="Wallet Reset WS")
    db.add(ws)
    db.commit()

    # Initial wallet: ₹500 included promo + ₹5,000 purchased = ₹5,500 total
    wallet = WCCWallet(
        workspace_id=ws.id,
        included_balance=Decimal("500.00"),
        purchased_balance=Decimal("5000.00"),
    ...
    # 1. Reset Purchased Balance only
    asyncio.run(reset_wallet(ws.id, ResetResourceRequest(target="purchased", reason="Reset Purchased Wallet"), db=db, admin_user="test_admin"))

    w = db.query(WCCWallet).filter(WCCWallet.workspace_id == ws.id).first()
    assert w.purchased_balance == Decimal("0.00")
    assert w.included_balance == Decimal("500.00")
    assert w.balance == Decimal("500.00")

    # 2. Simulate User Recharging ₹1,000 (standard recharge logic)
    recharge_taxable = Decimal("1000.00")
    w.purchased_balance = (w.purchased_balance or Decimal("0.00")) + recharge_taxable
    w.balance = (w.included_balance or Decimal("0.00")) + w.purchased_balance
    db.commit()

    # Verify: purchased_balance must be EXACTLY ₹1,000.00 (NOT ₹6,000.00!)
    w = db.query(WCCWallet).filter(WCCWallet.workspace_id == ws.id).first()
    assert w.purchased_balance == Decimal("1000.00")
    assert w.included_balance == Decimal("500.00")
    assert w.balance == Decimal("1500.00")

    # 3. Reset All Wallet
    asyncio.run(reset_wallet(ws.id, ResetResourceRequest(target="all", reason="Reset All Wallet"), db=db, admin_user="test_admin"))
    ...
    # 4. User Recharges ₹1,000 again
    w.purchased_balance = (w.purchased_balance or Decimal("0.00")) + recharge_taxable
    w.balance = (w.included_balance or Decimal("0.00")) + w.purchased_balance
    db.commit()

    # Verify: Total balance is ₹1,000.00
    w = db.query(WCCWallet).filter(WCCWallet.workspace_id == ws.id).first()
    assert w.purchased_balance == Decimal("1000.00")
    assert w.included_balance == Decimal("0.00")
    assert w.balance == Decimal("1000.00")
